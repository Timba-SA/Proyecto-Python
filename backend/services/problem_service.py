...
class ProblemService:
    ...
    def get_problem_dir(self, problem_id: str) -> Path:
        """Get problem directory, raise if not exists (searches recursively)"""
        def find_problem_dir(base_dir: Path, target_name: str) -> Optional[Path]:
            """Recursively search for problem directory by name"""
            for item in base_dir.iterdir():
                if not item.is_dir() or item.name.startswith('.'):
                    continue

                # Check if this is the problem we're looking for
                if item.name == target_name and (item / "metadata.json").exists():
                    return item

                # Recurse into subdirectory
                result = find_problem_dir(item, target_name)
                if result:
                    return result

            return None

        pdir = find_problem_dir(self.problems_dir, problem_id)
        if not pdir:
    ...
    def _scan_problems_recursive(self, base_dir: Path) -> Dict[str, Dict[str, Any]]:
        """
        Recursively scan for problems in directory tree.
        ...
        def scan_dir(current_dir: Path):
            ...
            for item in current_dir.iterdir():
                ...
                if (item / "metadata.json").exists():
                    try:
                        problem_data = self._load_problem_data(item)
                        problems[item.name] = problem_data
                    except Exception as e:
                        logger.error(f"Error loading problem {item.name}: {e}")
                else:
                    # Recurse into subdirectory
                    scan_dir(item)
    ...
    def _list_all_cached(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        problems = self._scan_problems_recursive(self.problems_dir)
    ...
    def list_all(self) -> Dict[str, Dict[str, Any]]:
        ...
        if self._cache_enabled:
            ...
            return self._scan_problems_recursive(self.problems_dir)
