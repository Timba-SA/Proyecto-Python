...
class TestProblemService:
    ...
    def test_get_test_files_both_exist(self, mock_problem_dir, monkeypatch):
        ...
        test_files = service.get_test_files("sumatoria")

        assert test_files["public"] is not None
        assert test_files["public"].exists()
        assert test_files["public"].name == "tests_public.py"
        assert test_files["hidden"] is not None
        assert test_files["hidden"].exists()
        assert test_files["hidden"].name == "tests_hidden.py"
    ...
    def test_get_test_files_legacy_fallback(self, mock_problem_dir, monkeypatch):
        ...
        assert test_files["public"] is None
        assert test_files["hidden"] is None
        assert test_files["legacy"] is not None
        assert test_files["legacy"].exists()
        assert test_files["legacy"].name == "tests.py"
    ...
    def test_get_test_files_not_found(self, mock_problem_dir, monkeypatch):
        ...
        assert test_files["public"] is None
        assert test_files["hidden"] is None
        assert test_files["legacy"] is None
    ...
    def test_load_rubric_success(self, mock_problem_dir, monkeypatch):
        ...
        rubric = service.load_rubric("sumatoria")
    ...
    def test_load_rubric_file_not_found(self, mock_problem_dir, monkeypatch):
        """Test when rubric.json doesn't exist"""
        service = ProblemService()
        monkeypatch.setattr(service, 'problems_dir', mock_problem_dir)
        problem_dir = mock_problem_dir / "sumatoria"
        ...
        # Should return empty rubric with defaults
        assert rubric["tests"] == []
