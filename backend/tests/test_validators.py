...
class TestValidateCodeLength:
    ...
    def test_valid_code_length(self):
        ...
        code = "def suma(a, b):\n    return a + b"
        # Should not raise (uses settings.MAX_CODE_LENGTH = 50000)
        validate_code_length(code)

    def test_code_too_long(self):
        """Test code exceeding length limit"""
        from backend.config import settings
        code = "x" * (settings.MAX_CODE_LENGTH + 1)
    ...
    def test_code_exactly_at_limit(self):
        ...
        from backend.config import settings
        code = "x" * settings.MAX_CODE_LENGTH
        # Should not raise
        validate_code_length(code)
...
class TestValidateProblemExists:
    ...
    def test_problem_exists(self, mock_problem_dir, monkeypatch):
        ...
        monkeypatch.setattr(problem_service, 'problems_dir', mock_problem_dir)
        # Should not raise
        validate_problem_exists("sumatoria")
    ...
    def test_problem_not_exists(self, mock_problem_dir, monkeypatch):
        ...
        with pytest.raises(ValidationError) as exc_info:  # Changed from ProblemNotFoundError
            validate_problem_exists("nonexistent_xyz")
...
class TestValidateSubmissionRequest:
    ...
    def test_problem_not_exists(self, mock_problem_dir, monkeypatch):
        ...
        with pytest.raises(ValidationError):  # Changed from ProblemNotFoundError
            validate_submission_request(req)
    ...
    def test_code_too_long(self, mock_problem_dir, monkeypatch):
        ...
        req = SubmissionRequest(
            problem_id="sumatoria",
            code="x" * (settings.MAX_CODE_LENGTH + 1),  # Very long code
            student_id="test"
        )
